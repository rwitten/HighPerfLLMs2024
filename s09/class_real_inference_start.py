...
def attention_ourselves(_Q, _K, _V, seq_pos):
    query_segment_id = jnp.ones( (1,1), dtype=jnp.int32)
    kv_segment_id = jnp.ones( (1, SEQUENCE_LENGTH), jnp.int32) * jnp.expand_dims(jnp.arange(SEQUENCE_LENGTH) <= seq_pos, axis = 0)

    #print(query_segment_id, kv_segment_id)
...
class OurModel(nn.Module):
  @nn.compact
  def __call__(self, x, cache, seq_pos):
    '''
        x is [BATCH, SEQUENCE]
    '''
    ...
    for i in range(LAYERS):
      ...
      q = jnp.einsum("BSE,EHD->BSHD",x, q_proj, preferred_element_type = jnp.float32)
      ...
      k = jnp.einsum("BSE,EHD->BSHD",x, k_proj, preferred_element_type = jnp.float32 )

      cache[f"key_{i}"] = jax.lax.dynamic_update_index_in_dim(cache[f"key_{i}"], k, seq_pos, 1)
      k = cache[f"key_{i}"]
      ...
      v = jnp.einsum("BSE,EHD->BSHD",x, v_proj, preferred_element_type = jnp.float32  )
      cache[f"value_{i}"] = jax.lax.dynamic_update_index_in_dim(cache[f"value_{i}"], v, seq_pos, 1)
      v = cache[f"value_{i}"]

      o = attention_ourselves(q,k,v,seq_pos)
...
def init_kv_cache():
    cache = {}
    for i in range(LAYERS):
        cache[f"key_{i}"] = jnp.zeros( (1,SEQUENCE_LENGTH,NUM_HEADS,HEAD_DEPTH), dtype=jnp.float32)
        cache[f"value_{i}"] = jnp.zeros( (1,SEQUENCE_LENGTH,NUM_HEADS,HEAD_DEPTH), dtype=jnp.float32)
    return cache
...
def main():
    ...
    number_total_vars = calculate_num_params(_params) + calculate_num_params(kv_cache)
    bytes_total_vars = 4 * number_total_vars
    print(f"Number total params {number_total_vars/1e6} million, {bytes_total_vars/1e9} GB")
    ...
    print(f"achieved bandwidth {bytes_total_vars/1e9/(time_ms/1000)} GB/s")
    ...
    output_string = ""
    for i in range(NUM_TOKENS):
        output_string += chr(output_tokens[i].item())
